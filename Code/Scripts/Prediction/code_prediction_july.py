...
pooch_data = pooch.create(
    path=pooch.os_cache("BikeProjectTeam7"),  # Cache des fichiers localement
    base_url=BASE_URL,                       # URL de base pour télécharger les fichiers
    registry={                               # Fichiers à récupérer
        "fichier_combined.csv": "sha256:08c71a1718b279efe1ebb60f6446e19c8b786d93a2a16bcb8504ab1a888dc3f8"
    },
)

# Télécharger et charger le fichier de données
file_path = Path(pooch_data.fetch("fichier_combined.csv"))  # Récupérer le fichier avec Pooch
data = pd.read_csv(file_path, sep=';')                     # Charger les données avec pandas
...
os.makedirs(prediction_folder, exist_ok=True)  # Créer le dossier si nécessaire
predictions_path = prediction_folder / 'predictions_bike_intensity_july_week.csv'
predictions_df.to_csv(predictions_path, index=False, sep=';')
