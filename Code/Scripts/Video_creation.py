...
if __name__ == "__main__":
    create_bike_animation(
        input_file='/home/samira2050/HAX712X/BikeProjectTeam7/Code/Data/VideoDatacleaned.csv',
...
def count_trips_per_day(input_file, target_date=None):
    """
    Compte le nombre de trajets pour chaque jour ou pour une date spécifique.
    
    Args:
        input_file (str): Chemin vers le fichier CSV contenant les données.
        target_date (str): Date cible au format "YYYY-MM-DD". Si None, retourne le décompte pour tous les jours.
    
    Returns:
        pd.DataFrame: DataFrame contenant le nombre de trajets par jour.
    """
    # Charger les données
    df = pd.read_csv(input_file)
    
    # Convertir la colonne de date/heure en datetime
    df['Departure_DateTime'] = pd.to_datetime(df['Departure_Date'] + ' ' + df['Departure_Time'])
    
    # Extraire les dates uniquement
    df['Departure_Date'] = df['Departure_DateTime'].dt.date
    
    # Si une date cible est spécifiée, filtrer les données
    if target_date:
        target_date = pd.to_datetime(target_date).date()
        df = df[df['Departure_Date'] == target_date]
    
    # Compter les trajets par jour
    trip_counts = df.groupby('Departure_Date').size().reset_index(name='Trip_Count')
    
    return trip_counts

# Exemple d'utilisation
if __name__ == "__main__":
    input_file = '/home/samira2050/HAX712X/BikeProjectTeam7/Code/Data/VideoDatacleaned.csv'  # Remplacez par votre chemin
    target_date = "2023-07-10"  # Exemple de date cible, None pour tous les jours
    
    trip_counts = count_trips_per_day(input_file, target_date=target_date)
    print(trip_counts)
