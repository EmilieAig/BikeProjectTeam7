...
from bs4 import BeautifulSoup # Transform the HTML content into a navigable structure, making it easier to extract file-specific links
import glob
import json
import pandas as pd
...
# Retrieve the webpage containing the list of files
response = requests.get(page_url)
soup = BeautifulSoup(response.text, 'html.parser')

# Find all file links that meet the criteria
file_links = []
...
# Download files that match the criteria
for url in file_links:
    ...
    if os.path.exists(full_path):
        os.remove(full_path)
        print(f"Deleting existing file: {full_path}")
